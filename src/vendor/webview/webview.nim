...
when defined(useWebviewDll):
  ...
  when defined(vcc):
    ...
    when defined(linux) or defined(bsd) or defined(webviewGtk):
      ...
      # ZMarkdown vendored change: Ubuntu 24.04 (Noble) ships only
      # webkit2gtk-4.1; the 4.0 dev package is gone. Default to 4.1 and allow
      # overriding the pkg-config module name at compile time with
      # -d:webkitGtkModule:webkit2gtk-4.0 for older distros.
      const webkitGtkModule {.strdefine.} = "webkit2gtk-4.1"
      {.passL: staticExec("pkg-config --libs gtk+-3.0 " & webkitGtkModule).}
      {.passC: staticExec("pkg-config --cflags gtk+-3.0 " & webkitGtkModule).}
...
# ZMarkdown vendored change: keep every binding context reachable so the GC
# does not free one while the webview still holds its pointer.
var keepAliveContexts {.global.}: seq[CallBackContext]
...
proc bindCallback*(w: Webview; name: string;
  ...
  # ZMarkdown vendored change: Nim 2.2 rejects initializing a {.global.} from
  # proc-local values ("cannot assign local to global variable"). Split the
  # declaration and assignment so it compiles. The contexts are also kept alive
  # in a module-level seq so multiple bindings do not clobber a single global.
  var arg {.global.}: CallBackContext
  arg = CallBackContext(w: w, fn: fn)
  keepAliveContexts.add(arg)
